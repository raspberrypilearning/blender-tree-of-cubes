...
+ Click somewhere else in the 3D view (but still close to the tree trunk) and add another cube. Například:
...
+ Move the cubes on top of the tree trunk in a tree-like arrangement. Například:

![Blender tree](images/blender-tree-1.png)

+ Rotate the 3D view using the middle mouse button and check that the tree looks okay from all angles, adjusting where necessary. Například:
